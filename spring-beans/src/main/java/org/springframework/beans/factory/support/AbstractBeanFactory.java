...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		//Spring创建bean的原则是不等bean创建完成就会将创建的bean的ObjectFactory提早曝光，也就是把ObjectFactory加入到缓存中，一旦下个bean创建时需要上个bean则直接使用ObjectFactory
		...
		if (sharedInstance != null && args == null) {
			...
			//返回相应的实例，(通过缓存只可以拿到bean的原始状态，还需要对bean进行实例化)
			...
			//如果beanDefinitionMap(已经加载过的类)中不包含beanName，则需要尝试从parentBeanFactory处理
...
//从这里开始创建bean，先进行记录
			...
			try {
				//将存储XML配置文件的GenericBeanDefinition转化为RootBeanDefinition;转化的时候如果父类不为空会合并父类的属性
			final RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
				...
				// Create bean instance.真正的开始创建bean
				if (mbd.isSingleton()) {
					//判断是否单例
